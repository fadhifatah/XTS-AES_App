...
public class Util {
    ...
    public static final int SPLIT_KEY_SIZE = 128;
    ...
    public static int[] file2int(File file) throws IOException {
        ...
        // alokasi sebagian data (sepanjang 16384) yang akan dibaca dari stream
        int sizeOfData;
        byte[] dataAllocation = new byte[16384];

        // memasukkan data dari stream ke dalam buffer
        while ((sizeOfData = stream.read(dataAllocation)) != -1)
            buffer.write(dataAllocation, 0, sizeOfData);
        buffer.flush();
     ...
     * @throws IOException
     */
    public static void int2file(int[] data, File file) throws IOException {
        FileOutputStream stream = new FileOutputStream(file);

        byte[] dataInBytes = new byte[data.length];
        for (int i = 0; i < dataInBytes.length; i++)
            dataInBytes[i] = (byte) (data[i] - 128);
